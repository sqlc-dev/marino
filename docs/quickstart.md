...
Parser implements the interface [`ast.Node`](https://pkg.go.dev/github.com/sqlc-dev/marino/ast?tab=doc#Node) for each kind of AST node, such as `SelectStmt`, `TableName`, `ColumnName`, etc. [`ast.Node`](https://pkg.go.dev/github.com/sqlc-dev/marino/ast?tab=doc#Node) provides a method `Accept(v Visitor) (node Node, ok bool)` to allow any struct that has implemented [`ast.Visitor`](https://pkg.go.dev/github.com/sqlc-dev/marino/ast?tab=doc#Visitor) to traverse itself.

[`ast.Visitor`](https://pkg.go.dev/github.com/sqlc-dev/marino/ast?tab=doc#Visitor) is defined as follows:
```go
