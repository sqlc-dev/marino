...
func TestKeywordsLength(t *testing.T) {
	if !reflect.DeepEqual(728, len(parser.Keywords)) {
		t.Fatalf("got %v, want %v", len(parser.Keywords), 728)
	}
	...
	if !reflect.DeepEqual(240, reservedNr) {
		t.Fatalf("got %v, want %v", reservedNr, 240)
	}
}
