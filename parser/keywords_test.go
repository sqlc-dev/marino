...
func TestKeywordsLength(t *testing.T) {
	if !reflect.DeepEqual(784, len(parser.Keywords)) {
		t.Fatalf("got %v, want %v", len(parser.Keywords), 784)
	}
	...
	if !reflect.DeepEqual(248, reservedNr) {
		t.Fatalf("got %v, want %v", reservedNr, 248)
	}
}
