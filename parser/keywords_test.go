...
func TestKeywordsLength(t *testing.T) {
	if !reflect.DeepEqual(718, len(parser.Keywords)) {
		t.Fatalf("got %v, want %v", len(parser.Keywords), 718)
	}
	...
	if !reflect.DeepEqual(239, reservedNr) {
		t.Fatalf("got %v, want %v", reservedNr, 239)
	}
}
